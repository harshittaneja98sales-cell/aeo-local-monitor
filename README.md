...
- Postgres-backed business records and saved audit runs when `DATABASE_URL` is configured.
...
The database layer is compatible with managed Postgres providers such as Supabase, Neon, and Vercel Postgres. The API creates the required tables automatically on first use; the same schema is also checked in at `database/schema.sql`.
